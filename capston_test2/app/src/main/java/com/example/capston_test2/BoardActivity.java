...
//jdbc_test 테이블에서 contents 값을 웹뷰에 찍는 자바파일 ListActivit.java에서 post_id값 가져옴 + 추천하기 + 댓글보기 + 파일 다운로드
public class BoardActivity extends Activity {
    private Button button14; //추천하기 버튼
    private Button button17; //댓글보기 버튼
    private TextView textViewBoardTitle; //게시글 내용 이라는 타이틀
    private WebView webViewPostContent; //내용 나오는 웹 뷰
    private WebView textView7;// 조회수 웹뷰
    private WebView textView6; //추천수 웹뷰
    /*--------------------------------------------------------------------*/
    private Button buttonDownload;//파일 다운로드 버튼
    ...
    private void fetchFileNameAndDownload() {
        ...
        new FetchFileNameTask().execute("http://shinhan-software.co.kr/mobile_filedownload.jsp?post_id=" + postId);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_list_1); // 해당 layout 파일을 생성해야 합니다.
        ...
        buttonDownload = findViewById(R.id.buttonDownload); // 레이아웃에서 Button을 찾습니다.
        ...
        textView7.getSettings().setJavaScriptEnabled(true); // 자바스크립트를 활성화합니다.
        textView7.setWebViewClient(new WebViewClient()); // 새로운 WebViewClient를 설정하여 기본 웹 브라우저에서 열리는 것을 방지합니다.

        textView6.getSettings().setJavaScriptEnabled(true); // 자바스크립트를 활성화합니다.
        textView6.setWebViewClient(new WebViewClient()); // 새로운 WebViewClient를 설정하여 기본 웹 브라우저에서 열리는 것을 방지합니다.
        ...
        webViewPostContent.loadUrl("http://shinhan-software.co.kr/mobile_contents.jsp?post_id=" + postId);

        textView7.loadUrl("http://shinhan-software.co.kr/mobile_views.jsp?post_id=" + postId);
        textView6.loadUrl("http://shinhan-software.co.kr/mobile_recommand.jsp?post_id=" + postId);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // API URL 준비
                    URL url = new URL("http://shinhan-software.co.kr/mobile_filedownload.jsp?post_id=" + postId);
        ...
        button14.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 1초 지연 후에 토스트 알림을 추가하여 사용자에게 알립니다.
                new android.os.Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        // 1초 지연 후에 textView7와 textView6 WebView를 새로 고침합니다.
                        textView7.loadUrl("http://shinhan-software.co.kr/mobile_views.jsp?post_id=" + postId);
                        textView6.loadUrl("http://shinhan-software.co.kr/mobile_recommand.jsp?post_id=" + postId);
                        Toast.makeText(BoardActivity.this, "추천되었습니다!", Toast.LENGTH_SHORT).show();
                    }
                }, 1000); // 1000 밀리초(1초) 지연
            }
        });
        ...
        button17.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(BoardActivity.this, ReplyListActivity.class);
                intent.putExtra("re_postId", postId);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_SAVE_FILE && resultCode == Activity.RESULT_OK) {
            if (data != null) {
                ...
                // 파일 다운로드 및 저장 시작
                new DownloadFileTask(uri).execute("http://shinhan-software.co.kr/filedownload.jsp?post_id=" + postId);
            }
        }
    }
    ...
    private class DownloadFileTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            Toast.makeText(BoardActivity.this, result, Toast.LENGTH_LONG).show();
    ...
    private class FetchFileNameTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if (result != null) {
                ...
                Toast.makeText(BoardActivity.this, "Error fetching file name", Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private class IncrementViewsTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... postIds) {
            try {
                ...
                URL url = new URL("http://shinhan-software.co.kr/mobile_update_recommand.jsp?post_id=" + postId);
